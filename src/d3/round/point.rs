...
impl<T: Field + Copy> Outer for Point<T> {
	...
	fn outer(self, rhs: Self) -> Pair<T> {
		let R410{ e12, e13, e23, e1p, e1n, e2p, e2n, e3p, e3n, epn, .. } = self.into_multivec()^rhs.into_multivec();
		Pair{ e12, e13, e23, e1p, e1n, e2p, e2n, e3p, e3n, epn }
		/*
		Pair {
...
mod test {
	...
	fn test_pair() {
		...
		assert_eq!(p1, Point { e1: 1.0, e2: 2.0, e3: 4.0, ep: 10.0, en: 11.0 });
		assert_eq!(p2, Point { e1: 0.0, e2: 3.0, e3: 0.0, ep: 4.0, en: 5.0 });
	...
	fn test_pair_fast() {
		...
		assert_eq!(p1, Point { e1: F32::new(1.0), e2: F32::new(2.0), e3: F32::new(4.0), ep: F32::new(10.0), en: F32::new(11.0) });
		assert_eq!(p2, Point { e1: F32::new(0.0), e2: F32::new(3.0), e3: F32::new(0.0), ep: F32::new(4.0), en: F32::new(5.0) });
