...
use simba::simd::{SimdRealField as Field, SimdOption};
use simba::scalar::RealField;
use num_traits::{Zero, zero, one};

use crate::{Outer, Inner};
use super::{Point, Circle};
...
impl<T: Field> Inner for Pair<T> {
	...
	fn inner(self, rhs: Self) -> T {
		...
		- self.e1p * rhs.e1p - self.e2p * rhs.e2p - self.e3p * rhs.e3p
		+ self.e1n * rhs.e1n + self.e2n * rhs.e2n + self.e3n * rhs.e3n + self.epn * rhs.epn 
	}
}
